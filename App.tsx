...
const App: React.FC = () => {
    const [view, setView] = React.useState<'METRONOME' | 'LOOP_TRAINER'>('METRONOME');
    ...
    // Auto-stop metronome when switching views
    React.useEffect(() => {
        if (view !== 'METRONOME' && isPlaying) {
            toggle();
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-white flex flex-col overflow-hidden">
            ...
            <main className="relative flex-1 w-full max-w-4xl mx-auto flex flex-col items-center justify-between px-6 py-6 overflow-visible">
                ...
                <div className="w-full max-w-md flex justify-center mb-4">
                    <div className="flex bg-slate-900/50 p-1 rounded-xl border border-white/5 backdrop-blur-sm shadow-lg">
                        <button
                            onClick={() => setView('METRONOME')}
                            className={`px-6 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-300 ${view === 'METRONOME'
                                ? 'bg-gradient-to-r from-cyan-500/20 to-cyan-400/10 text-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.2)] border border-cyan-500/20'
                        ...
                        <button
                            onClick={() => setView('LOOP_TRAINER')}
                            className={`px-6 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-300 ${view === 'LOOP_TRAINER'
                                ? 'bg-gradient-to-r from-purple-500/20 to-purple-400/10 text-purple-400 shadow-[0_0_10px_rgba(168,85,247,0.2)] border border-purple-500/20'
                                : 'text-slate-500 hover:text-slate-300'
                                }`}
                        >
                            Loop Trainer
                ...
                {view === 'METRONOME' ? (
                    ...
                    <LoopTrainer onBack={() => setView('METRONOME')} />
                )}
            </main>
